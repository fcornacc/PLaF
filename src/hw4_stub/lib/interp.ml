(* Frank Cornacchiulo, Siddharth Balaji *)
...
and
  ...
  | Proj(e,id) ->
    ...
    find_field l id >>= fun (b, v) ->
    if b=false
    then return v
    else int_of_refVal v >>= fun x ->
         Store.deref g_store x
  | SetField(e1,id,e2) ->
    eval_expr e1 >>=
    fields_of_recordVal >>= fun l ->
    find_field l id >>= fun (b, v) ->
    if b=false
    then error "Field not mutable"
    else int_of_refVal v >>= fun x ->
         eval_expr e2 >>= fun n ->
         Store.set_ref g_store x n >>= fun _ ->
         return UnitVal 
  | IsNumber(e) -> 
    eval_expr e >>=
    bool_of_numVal >>= fun b ->
    return (BoolVal b)
  | Unit -> return UnitVal
  | Debug(_e) ->
