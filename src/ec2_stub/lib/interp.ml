...
let rec ancestors : string -> class_env -> string list =
  fun c_name c_env ->
  c_name :: 
  (match List.assoc_opt c_name c_env with
  | None -> []
  | Some (super,_fields,_methods) -> ancestors super c_env)

let is_subclass : string -> string -> class_env -> exp_val ea_result = 
  fun c_name1 c_name2 c_env ->
  if List.mem_assoc c_name2 !g_class_env
  then return (BoolVal (List.mem c_name2 @@ ancestors c_name1 c_env))
...
and   
  ...
  | NewObject(c_name,es) ->
    ...
    | None -> return self
    | Some m -> apply_method "initialize" self args m
  >>= fun _ ->return self))
  | Send(e,m_name,es) ->
