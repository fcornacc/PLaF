...
(* Changed so that it takes a list of procs instead of a single decleration *)
let extend_env_rec = 
  fun pl env ->
    Ok (ExtendEnvRec(pl, env))

(* Checks the list of procs and will return none if nothing is found in the list
   but will return the matching proc if it is found *)

let rec helper = 
  fun id l -> 
    match l with
    | [] -> None
    | (v, par, op1, op2, body)::t -> 
      if id = v 
      then Some (v, par, op1, op2, body) 
      else helper id t

(* Changed apply_env so that ExtendEnvRec takes a list of procs and will check if
    there is a matching id in the list using a helper and will return the environment including the
    entire list or recurse through the rest of the environment *)
...
let rec apply_env : string -> exp_val ea_result =
  ...
  | ExtendEnvRec (procs , tail) ->
    match (helper id procs) with
    | None -> apply_env id tail                   
    | Some (_,par,_,_,body) -> Ok (ProcVal(par,body,env))
